...
CON_COMMAND( AMP_C_DoAskConnect, "Server plugin example of using the ask connect dialog" )
{
	if ( args.ArgC() < 2 )
	{
...
//---------------------------------------------------------------------------------
CON_COMMAND(AMP_C_empty_version, "prints the version of the empty plugin")
{
Msg( "CLIENT TEST MANU Version:2.0.0.0\n" );
}

CON_COMMAND(AMP_C_empty_log, "logs the version of the empty plugin")
{
